...
export const ExpertCard: React.FC<ExpertCardProps> = ({ service }) => {
  return (
    <div className="water-surface-card bg-white rounded-2xl border border-timberwolf/70 p-6 flex flex-col justify-between hover:border-moonstone/60 transition-all duration-200 group">
      <div>
        {/* Header: Avatar, Name, Verification, Rating */}
        <div className="flex items-start gap-4 mb-4">
          <div className="relative">
            ...
            {service.available_now && (
              <span
                ...
                title="Available now for instant booking"
              />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-midnight/70 font-medium truncate mt-0.5">
      ...
      <div className="pt-4 border-t border-timberwolf/40 flex items-center justify-between">
        <div>
          <div className="flex items-baseline gap-1">
            <span className="font-bold text-lg text-midnight">
              ${service.price_per_minute.toFixed(2)}
            </span>
            <span className="text-xs text-midnight/60">/ min</span>
          </div>
          <span className="text-[11px] text-midnight/50 block mt-0.5">
            {service.available_now ? 'Available today' : 'By appointment'}
          </span>
        </div>

        <Link
          to={`/services/${service.id}`}
          className="btn-shine inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-midnight text-aliceblue text-xs font-semibold hover:bg-midnight-hover transition-all shadow-subtle group-hover:bg-moonstone group-hover:text-white cursor-pointer"
        >
